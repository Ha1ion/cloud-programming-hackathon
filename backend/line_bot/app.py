...
def handle_message(event):
    ...
    # 預設的假 event_id (黑客松 Demo 用)
    mock_event_id = "evt_001"
    backend_api_url = f"http://localhost:8080/events/{mock_event_id}/status" # 假設的後端 API 網址

    if user_msg == "我沒事":
        # 模擬打 API 給後端更新狀態
        print(f"[{user_id}] 傳送 API 至後端: 更新狀態為 SAFE, 取消警報")
        # requests.post(backend_api_url, json={"status": "SAFE"})
        reply_text = "🟢 已收到您的回報！目前狀態已更新為：安全 (SAFE)。\n警報已解除，請安心騎乘！"
        
    elif user_msg == "需要協助":
        print(f"[{user_id}] 傳送 API 至後端: 觸發緊急通報 EMERGENCY")
        # requests.post(backend_api_url, json={"status": "EMERGENCY"})
        ...
        reply_text = "🗺️ 【校園安全 Dashboard】\n\n點擊下方連結查看即時事故熱點與高風險路段視覺化地圖：\nhttps://your-react-dashboard.com"

    elif user_msg == "設定緊急聯絡人":
